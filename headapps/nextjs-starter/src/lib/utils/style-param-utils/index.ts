...
/**
 * Aliases for `ctaVariant` values emitted by Sitecore that don't match the
 * canonical variant names in `CtaVariantValues` (`fill | outline | ghost | link`).
 *
 * Some content items in Sitecore use author-friendly labels (`primary`, `secondary`)
 * or legacy values (`filled`). This map normalizes them so the parser stays the
 * single source of truth and we don't need a content migration to fix the drift.
 */
const CTA_VARIANT_ALIASES: Record<string, string> = {
  filled: 'fill',
  primary: 'fill',
  secondary: 'outline',
  tertiary: 'ghost',
 ...
 *    props.params.Styles === 'cta1:ctaVariant:filled cta1:ctaIcon:arrow-right cta2:ctaVariant:outline cardsPerRow:1'
 *    // Parse the styles
...
export function parseStyleParams<TElement extends ValidElements>(
  ...
  selectedStylesString.split(' ').forEach((rawValue) => {
    ...
    const originalValue = split[2];

    const value =
      styleType === 'ctaVariant' && CTA_VARIANT_ALIASES[originalValue]
        ? CTA_VARIANT_ALIASES[originalValue]
        : originalValue;
