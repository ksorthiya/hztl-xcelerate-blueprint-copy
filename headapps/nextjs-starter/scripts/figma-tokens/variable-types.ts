...
export function getVariableType(
  ...
  if (key.match(/spacing|padding|margin|space-between|space between|layout|gutter|gap/i)) {
    return 'spacing';
  }
  ...
  if (key.match(/radius/i) || extensionScope?.includes('CORNER_RADIUS')) {
  ...
  if (
    key.match(
      /component-.*-button-type|component-.*-fill|component-.*-item-button-type|component-.*-logo|brand-name/i
    )
  ) {
    return 'IGNORED';
  }
  ...
  if (key.match(/component-header-batch|component-header-name|component-header-not-started/i)) {
    return 'IGNORED';
  }
  throw new Error(
    `Unknown variable type: ${token.$type} ${extensionScope?.join('|')} ${key}, ${token.$value}`
  );
}
