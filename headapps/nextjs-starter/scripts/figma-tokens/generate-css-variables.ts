...
const themeConfig = {${EOL}`;
  ...
  const { globalConfig, deviceConfigs, brandConfigs, themeConfigs, styleConfigs, spacingConfigs } =
  ...
  if (styleConfigs.length > 0) {
    themeConfigFileContent += `  Style: ${styleConfigs[0]},${EOL}`;
  }
  if (spacingConfigs.length > 0) {
    themeConfigFileContent += `  Spacing: ${spacingConfigs[0]},${EOL}`;
  }
...
async function writeCssFiles(jsonFiles: { [key: string]: any }) {
  ...
  const fileKeys = Object.keys(jsonFiles);
  // const globalKeys = fileKeys.filter((key) => key.startsWith('Global'));
  const deviceKeys = fileKeys.filter((key) => key.startsWith('Device'));
  const brandKeys = fileKeys.filter((key) => key.startsWith('Brand'));
  const themeKeys = fileKeys.filter((key) => key.startsWith('Theme'));
  const globalKeys = without(fileKeys, ...deviceKeys, ...brandKeys, ...themeKeys);

  for (let i = 0; i < globalKeys.length; i++) {
    const key = globalKeys[i];
    indexFileContent = await writeCssFile(jsonFiles, key, 'global', indexFileContent);
  }
  for (let i = 0; i < deviceKeys.length; i++) {
    const key = deviceKeys[i];
    indexFileContent = await writeCssFile(jsonFiles, key, 'device', indexFileContent);
  }
  for (let i = 0; i < brandKeys.length; i++) {
    ...
    if (
      ...
      indexFileContent = await writeCssFile(jsonFiles, key, 'brand', indexFileContent);
    }
  }
  for (let i = 0; i < themeKeys.length; i++) {
    const key = themeKeys[i];
    indexFileContent = await writeCssFile(jsonFiles, key, 'theme', indexFileContent);
  }
...
async function writeCssFile(
  ...
  key: string,
  keyType: 'global' | 'device' | 'brand' | 'theme',
  ...
  if (Object.prototype.hasOwnProperty.call(jsonFiles, key)) {
    ...
    } else if (keyType === 'device') {
      const minWidth = parsed['screen-dimensions-min-width'];
      if (key === 'DeviceDesktop') {
        fileContent = `@media (min-width: ${minWidth}) {
  .brand-root {${EOL}`;
      } else {
        const maxWidthRaw = parsed['screen-dimensions-max-width'];
        const maxWidth =
          typeof maxWidthRaw === 'string' && maxWidthRaw.endsWith('px')
            ? maxWidthRaw
            : `${maxWidthRaw}px`;
...
function splitJsonFileConfigs(jsonFiles: { [key: string]: any }) {
  const collectionKeys = Object.keys(jsonFiles);
  const deviceKeys = collectionKeys.filter((key) => key.startsWith('Device'));
  const brandKeys = collectionKeys.filter((key) => key.startsWith('Brand'));
  const themeKeys = collectionKeys.filter((key) => key.startsWith('Theme'));
  const styleKeys = collectionKeys.filter((key) => key.startsWith('Style'));
  const spacingKeys = collectionKeys.filter((key) => key.startsWith('Spacing'));
  const globalKeys = without(
    collectionKeys,
    ...deviceKeys,
    ...brandKeys,
    ...themeKeys,
  ...
  themeConfigs.forEach((config) => {
    if (config !== themeConfigs[0]) {
      console.log(themeConfigs.length, themeKeys);
      throw new Error('Theme configs must be the same');
    }
  });

  const styleConfigs = styleKeys.map(getJson);
  const spacingConfigs = spacingKeys.map(getJson);
