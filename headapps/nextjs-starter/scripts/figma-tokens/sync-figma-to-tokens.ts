...
import { Api as FigmaApi } from './figma_api';
...
async function main() {
  ...
  const api = new FigmaApi({ personalAccessToken: process.env.PERSONAL_ACCESS_TOKEN });
  const localVariables = await api.getLocalVariables({ file_key: fileKey });
  ...
  if (!fs.existsSync(`${outputDir}/raw`)) {
    fs.mkdirSync(`${outputDir}/raw`);
    console.log(`Created ${outputDir}/raw directory`);
  }
  fs.writeFileSync(
    `${outputDir}/raw/local-variables.json`,
  ...
  Object.entries(flatTokens).forEach(([fileName, fileContent]) => {
    if (!fs.existsSync(`${outputDir}/diff/${fileName}`)) {
      return;
    }
