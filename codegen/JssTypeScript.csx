...
public string RenderImportStatements()
{
    ...
    if(!Templates.Any()) {
        return "// No templates for this module";
    }
    ...
    localCode.AppendLine($@"// @ts-ignore ");
    localCode.AppendLine($@"import {{ Field, ImageField, FileField, LinkField, Item }} from '@sitecore-content-sdk/nextjs';");
    // Shouldn't need ts-ignore anymore since we fixed the TS issue.
    // localCode.AppendLine($@"// @ts-ignore");
    ...
    foreach (var template in GetBaseTemplates(Templates))
    {
        ...
        var nameSpaceAlias = GetNameSpaceAlias(template);
        var split = template.RootNamespace.Split('.');
        var layer = split[split.Length - 2];
        var module = split[split.Length - 1];
        localCode.AppendLine($@"// Template: {template.RootNamespace} {template.Name} {shortNameSpaceRoot}");
        if (!foundModules.Contains(nameSpaceAlias))
        {
            foundModules.Add(nameSpaceAlias);
    
            localCode.AppendLine($@"// @ts-ignore");
            localCode.AppendLine($@"import {{ {shortNameSpaceRoot} as {nameSpaceAlias} }} from ""./{module}.model""");
        } else {
            localCode.AppendLine($@"// Template: {shortNameSpaceRoot} already imported");
        }
    }
...
public string RenderTemplates()
{
    ...
    */
    export type {template.CodeName} = {RenderBaseInterfaces(template)} {{");
        ...
        localCode.AppendLine($@"
    /**
    * Represents the template {template.Path} as an Item
    */
    export type {template.CodeName}_Item = Omit<Item, 'fields'> & {template.CodeName}");

        localCode.AppendLine($@"
    /**
    * Represents the template {template.Path} as a Component
    */
    export type {template.CodeName}_Component = ComponentProps & {template.CodeName}");
